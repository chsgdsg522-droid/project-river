...
export function PracticePage({ room, socket, profile, muted = false, chatEvent = null }) {
  ...
  return (
    <>
      ...
      {showLearning && <LearningPanel mode={room.mode} holeCards={self?.holeCards} board={room.game?.board} pot={room.game?.pot} callAmount={room.game?.legalActions?.callAmount} />}
    </>
  );
}
